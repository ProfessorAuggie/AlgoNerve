...
export const SortingView: React.FC<SortingViewProps> = ({ step }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-between p-4 bg-zinc-900/40 rounded-xl border border-zinc-800/80 backdrop-blur-sm">
      <div className="flex-1 w-full flex items-end justify-center gap-2 max-w-2xl min-h-[300px] px-4 py-8">
        {array.map((val, idx) => {
          ...
          if (sorted.includes(idx)) {
            ...
            barColorClass = 'bg-indigo-600/40 border-indigo-500/60';
            if (idx === mid) {
          ...
          return (
            ...
            >
              {/* Value Label */}
              <span className="text-xs font-semibold text-zinc-400 mb-1 opacity-80 group-hover:opacity-100 transition-opacity">
                {val}
              </span>
              ...
              {/* Highlight Badges */}
              {val === pivot && (
                <span className="absolute -bottom-6 text-[9px] bg-fuchsia-950 text-fuchsia-300 border border-fuchsia-800 px-1 rounded font-semibold">
                  Pivot
                </span>
              )}
            </div>
      ...
      {/* Legend Indicators */}
      <div className="w-full flex flex-wrap items-center justify-center gap-6 mt-6 pt-4 border-t border-zinc-800/60 text-xs font-medium">
        <div className="flex items-center gap-2">
          <div className="w-3.0 h-3.0 rounded bg-indigo-500/80 border border-indigo-400" />
          <span className="text-zinc-300">Unsorted</span>
        </div>
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-zinc-300">Swapping</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3.0 h-3.0 rounded bg-emerald-500/80 border border-emerald-400" />
          <span className="text-zinc-300">Sorted</span>
        </div>
        {pivot !== undefined && (
          <div className="flex items-center gap-2">
